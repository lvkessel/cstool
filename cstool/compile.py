...
def icdf(x, cdf, P):
    ...
    return np.interp(P, cdf[OK], x[OK]) * x.units
...
def compute_tcs_icdf(f, P, eval_x):
    ...
    cf = np.r_[0, cumtrapz(y, eval_x)]
    if cf[-1] <= 0:
        return 0 * eval_x.units*y.units, np.zeros_like(P) * eval_x.units
...
def compute_2d_tcs_icdf(function_data, eval_x, eval_y, # Function, plus x and y coordinates at which to sample
    ...
    # CIy_x[i,j] == int_{y_low(eval_x[i])}^{eval_y[j]} f(eval_x[i], y')
    ...
    for i in range(len(eval_x)):
        ...
        data[OK] = function_data[i,OK]
        CIy_x[i,1:] = cumtrapz(data, eval_y)

    # CIx[i] = int_{eval_x[0]}^{eval_x[i]} dx' int_{y_low(x')}^{y_high(x')} dy' f(x', y')
    #        = int_{eval_x[0]}^{eval_x[i]} dx' CIy_x[x', -1]
    CIx = np.zeros(len(eval_x))
    CIx[1:] = cumtrapz(CIy_x[:,-1], eval_x)

    # total_cs = int_{eval_x[0]}^{eval_x[-1]} dx' int_{y_low(x')}^{y_high(x')} dy' f(x', y')
    #          = CIx[-1]
    total_cs = CIx[-1]
    if total_cs <= 0:
        return 0 * eval_x.units * eval_y.units * function_data.units, \
    ...
    ###########################################################################
    # CDF for x
    # P[i] = int_{eval_x[0]}^{eval_x[i]} dx' p(x')
    #      = int_{eval_x[0]}^{eval_x[i]} dx' int_{y_low(x')}^{y_high(x')} f(x', y') / total_cs
    cPx = CIx / total_cs

    # CDF for y given x
    # Notation: xx == eval_x[i]
    # cPy_x[i,j] = int_{y_low(xx),eval_y[j]} dy' p(y' | xx)
    #            = int_{y_low(xx),eval_y[j]} dy' p(xx, y') / p(xx)
    #            = [int_{y_low(xx),eval_y[j]} dy' p(xx, y')] / [int_{y_low(xx)}^{y_high(xx)} dy'' p(xx, y'')]
    #            =                                f(xx, y')                                       f(xx, y'')
