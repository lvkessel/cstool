...
def elf_full_penn(elf, omega_max, N_omega, N_q):
	pool = Pool()
	elf_data = elf.get_serialized();
	result = np.zeros([N_omega, N_q])

	omega_min = elf.get_min_energy()
	q_min = np.sqrt(2*units.m_e*omega_min)/units.hbar
	q_max = np.sqrt(2*units.m_e*omega_max)/units.hbar

	omega = np.geomspace(omega_min.to(omega_max.units).magnitude,
		omega_max.magnitude, N_omega) * omega_max.units;
	q = np.r_[0, np.geomspace(q_min.to(q_max.units).magnitude,
		q_max.magnitude, N_q-1)] * q_max.units

	for iw, w in enumerate(omega):
		print("# Computing Penn ELF: %d%%" % (100*iw / len(omega)), end='\r');
